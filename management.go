...
const TRANSLATIONS={
  en:{
    ...
    'queue.title':'Account Queue','queue.description':'Account cards are sorted by the current scheduler priority. The first available account is preferred for the next Codex request.','metrics.nextAccount':'Next account','metrics.monthlyMode':'Monthly','metrics.lastSelected':'Last selected',
  ...
  'zh-CN':{
    ...
    'queue.title':'账号队列','queue.description':'账号卡片按当前调度优先级排序。第一个可用账号就是下一次 Codex 请求会优先选择的账号。','metrics.nextAccount':'下一账号','metrics.monthlyMode':'Monthly','metrics.lastSelected':'最近选择',
...
function resetCreditCount(value){if(currentLocale!=='en')return value+' 次';return value+' '+(value===1?'time':'times')}

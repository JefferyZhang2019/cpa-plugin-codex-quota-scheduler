...
function retryFallbackNode(target,container){const wrapper=node('div','retryFallback');wrapper.dataset.retryFallback='1';const modelField=node('label','field');modelField.append(node('span','',t('retry.fallback')));const modelInput=document.createElement('input');modelInput.setAttribute('list','retryModelOptions');modelInput.setAttribute('spellcheck','false');modelInput.dataset.retryFallbackModel='1';modelInput.value=(target&&target.model)||'';modelField.append(modelInput);const remove=node('button','ghost retryRemove','');remove.type='button';remove.textContent=t('retry.remove');remove.addEventListener('click',()=>{const siblings=container?container.querySelectorAll('[data-retry-fallback]'):[];if(siblings.length<=1){modelInput.value=''}else wrapper.remove()});wrapper.append(modelField,remove);return wrapper}

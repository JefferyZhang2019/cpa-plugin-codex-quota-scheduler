...
- **在管理页面手动刷新单个账号**时，若最新额度快照显示所有已知窗口都有剩余
  额度，则清除标记。手动刷新还会越过宿主侧的 `disabled`/`unavailable` 冷却
  标记，因此在操作员触发上游重置后，即使 CPA 仍保留自己的冷却，账号也能完成
  刷新。
- **后台刷新绝不单凭百分比清除标记。** 通用额度端点可能显示剩余 100% 而模型
  请求仍被上游 429（在 K12 计划账号上观察到），因此自动恢复必须依靠上面的
  真实请求证据。
